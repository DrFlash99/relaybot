...
# RelayBot - Simple Relay Service, modules/relayminecrafttoirc.py
#
...
def loadconfig(config, module):
	...
	for relaycfg in config:
		if not 'name' in relaycfg.attrib:
			log.warning('Relay (Log To IRC) config missing name attribute')
			continue
		if relaycfg.attrib['name'] in configs:
			log.warning('Duplicate Relay (Log To IRC) config name: ' + relaycfg.attrib['name'])
			continue

		name = relaycfg.attrib['name']
		conf = {'name': name, 'log': '', 'irc': '', 'minecraft': '', 'channels': []}

		log = relaycfg.findall('./log')
		if not log:
			log.warning('Relay (Log to IRC) ' + name + ' missing log configuration')
			continue
		if not 'file' in log[0].attrib:
			log.warning('Relay (Log to IRC) ' + name + ' Log configuration missing file attribute')
			continue

		conf['log'] = log[0].attrib['file']

		irc = relaycfg.findall('./irc')
		if not irc:
			log.warning('Relay (Log to IRC) ' + name + ' missing IRC configuration')
			continue
		if not 'name' in irc[0].attrib:
			log.warning('Relay (Log to IRC) ' + name + ' IRC configuration missing name attribute')
			continue
...
def applyconfig(loop, module):
	...
	for name in configs:
		conf = configs[name]
		log.info('Creating Relay (Log To IRC) ' + name)
	return
